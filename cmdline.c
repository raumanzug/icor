...
static void icor_init_cmdline(icor_cmdline_t *);

static void icor_set_filename(icor_fileinfo_t *, const char *);

static void icor_set_filetype(icor_fileinfo_t *, const char *);
...
int icor_cmdline_main(int argc, char **argv, icor_cmdline_t *pCmdline) {
  ...
  while ((opt = getopt_long(argc, argv, opt_string, long_options, &opt_idx)) !=
         -1) {
    ...
    case 0:
      ...
      } else if (!strcmp(long_options[opt_idx].name, "oformat")) {
  ...
  if (retval != -2) {
    ...
    if (!(pCmdline->output).filename) {
      retval = -1;
      fputs(
          "output filename missing.  Specify one with -o or --output option\n",
          stderr);
    ...
    if (!(pCmdline->x > 0)) {
      retval = -1;
      fputs("x dim should be greater than 0.  Specify a x dim greater than 0 "
            "with -x option!\n",
            stderr);
    }
    if (!(pCmdline->y > 0)) {
      retval = -1;
      fputs("y dim should be greater than 0.  Specify a y dim greater than 0 "
            "with -y option!\n",
            stderr);
    }
  }
...
void icor_init_fileinfo(icor_fileinfo_t *pFileinfo) {
  assert(pFileinfo);
...
void icor_init_cmdline(icor_cmdline_t *pCmdline) {
  assert(pCmdline);
...
void icor_set_filename(icor_fileinfo_t *pFileinfo, const char *filename) {
  assert(pFileinfo);
...
void icor_set_filetype(icor_fileinfo_t *pFileinfo, const char *filetype) {
  assert(pFileinfo);
